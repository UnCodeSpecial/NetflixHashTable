...
void readSingleEntry(const string &str, netflix_entry *newEnt) {
    ...
    newEnt->year = atoi(fields[4].c_str());
    newEnt->nextnode = NULL; 
...
void entryInsert(netflix_entry *newEnt) {
    ...
    //
    int hashkey = getHashKey(newEnt->title);

    if(entryHash[hashkey]==NULL)
    {
        entryHash[hashkey]=newEnt;     // if at that position there is no field then set that position
    }
    else
    {
        netflix* ptr = entryHash[hashkey];  // else iterate until the end of list and add the newFT there
...
bool entryFind(const string &title, netflix_entry &foundEnt, int &ncmp) 
{
    ...
    //
    if(entryHash[key]==NULL){   // if at that index no element is there then return false directly
    return false;
    }
    else{
        netflix* ptr =entryHash[key];
        while (ptr!=NULL)      // else iterate over that index's linked list and if found set that details and return true;
        {
            ncmp++;
            if(ptr->title==title){
                foundEnt=*ptr;
...
int main() {
    ...
    for (;;) {
        ...
        // to exit the loop.
        //
        cout<<"\nEnter a movie or show title (<return> to quit): ";
        getline (cin, mTitle);
        if(mTitle== "")
        {
            cout<<endl<<"Exiting...";
        ...
        cout<<endl;
        if(entryFind(mTitle, foundEnt, ncmp))
        {
            cout<<"\nComparisons:" <<ncmp<<endl;
